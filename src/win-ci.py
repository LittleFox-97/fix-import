...
def main() -> None:
    """Выполняет основную функцию скрипта.

    Функция проверяет последний аргумент командной строки и в зависимости от его
    значения выполняет различные действия. Если последний аргумент - 'install',
    вызывается функция `install_deps` для установки зависимостей. В противном
    случае вызывается функция `build` для выполнения процесса сборки.
    """
